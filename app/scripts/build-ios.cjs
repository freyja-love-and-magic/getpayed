...
//        - App Group entitlements patched back in
...
const entitlementsPath = path.join(genApple, 'gelder_iOS', 'gelder_iOS.entitlements');
...
if (!fs.existsSync(ipaSrc)) {
  ...
  const exportPlist = path.join(require('os').tmpdir(), 'gelder-export-options.plist');
